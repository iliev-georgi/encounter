...
def get_statuses(user_id, bearer_token):
    ...
    for user_status in user_statuses:
        ...
        to_annotate_list.append(
            ToAnnotate(user_status["id"], preview_url_localized, user_status["content"], parser.parse(user_status["created_at"]).strftime("%s"))
        )

    return to_annotate_list


def get_attached_media(media_url, bearer_token):
    r = requests.get(
                media_url,
                headers={
                    "Authorization": f"Bearer {bearer_token}"
...
def tag_media(status_id, content, bearer_token):
    ...
    requests.post(status_create_url, headers={"Authorization": f"Bearer {bearer_token}"}, data=payload)